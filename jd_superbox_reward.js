...
!(async () => {
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    ...
    $.count =0
        ...
        if(i>0){
          await get_superboxSupBoxHomePage($.encryptPin[i-1],"562")
        }else{
          await get_superboxSupBoxHomePage("","")
        }
        
        res = await get_superboxSubBoxRewardPage($.encryptPin[i])
        for (var p=0;p<res.data.length;p++){	
            //console.log(`\n\nhome:${res.data.length }\n`)
            taskDetail = res.data[p]
            if(taskDetail.rewardType=='2'){
              $.count+=parseFloat(taskDetail.discount)
              
            }else{
              console.log(`\n\n获得价值${taskDetail.discount}实物:${taskDetail.skuName}\n`)
            }
  ...
  function get_superboxSubBoxRewardPage(encryptPin){
    let linkId="Ll3Qb2mhCXSEWxruhv8qIw"
    let body={"linkId":linkId,"encryptPin":encryptPin};
    return new Promise((resolve) => {$.get(taskPostUrl2("superboxSubBoxRewardPage",body), async (err, resp, data) => {
        try {
          if (err) {
